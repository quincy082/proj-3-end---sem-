int updateCount_me(float X[],float Y[],int coinCount[])
{
	...
	if(((red_count - coinCount[2]) > 0) && X[0]!=2.0f) { toReturn = 1; isRed_me = 'y';score_4p[1]=score_4p[1]+3;}
...
int updateCount_other(float X[],float Y[],int coinCount[])
{
	...
	if(X[4]==2.0f)	white_count = white_count + 1;
	if(X[5]==2.0f)	{ red_count = red_count + 1; }
	if(chance[0]=='y' || chance[2]=='y')
	{
		if(((white_count - coinCount[1]) > 0) && X[0]!=2.0f) {toReturn = 1;score_4p[0]=score_4p[0]+2;}
		if(((red_count - coinCount[2]) > 0) && X[0]!=2.0f) {toReturn = 1;isRed_other = 'y';score_4p[0]=score_4p[0]+3;}
	}
	else if(chance[3]=='y')
	{
		if(((black_count - coinCount[0]) > 0) && X[0]!=2.0f) {toReturn = 1;score_4p[1]=score_4p[1]+2;}
...
void update_chance(float Vx[], float Vy[], char chance[])
{
	...
	char cchek='x';
	if(isStrikerHitByMe=='y' && chance[1]=='y')
	{
		for(i=0;i<6;i++)
		{
		...
		if(cchek=='x')
		{
			...
			if(j == 0)//lost my chance
			{
				if(X[0]==2.0f)//striker goes into the hole,penalty:return pocketed white coin to board
				{
					...
					if(l==2) { x_r=0.0f;y_r=-0.41f; }
					if(l==3) { x_r=0.0f;y_r=0.41f; }
					if(l==4) { x_r=0.41f;y_r=0.0f; }
					if(l==5) { x_r=-0.41f;y_r=0.0f; }
	...
	if(isStrikerHitByOther=='y' && chance[1]=='n')//yahan pe aa rha h jo aana nahi chahiye
	{
		for(i=0;i<6;i++)
		{
			if(Vx[i]!=0.0f || Vy[i]!=0.0f)
		...
		if(cchek=='x')
		{
			k = updateCount_other(X,Y,coinCount);//k is 1 if chance of other continues and 0 otherwise
			if(k == 0)
			{
			...
			if(k == 1)//chance of that palyer continues!
			{
				if(chance[0]=='y')
				{	
					chance[0]='y';
