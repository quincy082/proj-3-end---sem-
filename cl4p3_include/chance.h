...
void update_chance(float Vx[], float Vy[], char chance[])
{
	...
	char cchek='x';
	if(isStrikerHitByMe=='y' && chance[3]=='y')
	{
		for(i=0;i<6;i++)
		{
		...
		if(cchek=='x')
		{
			...
			if(j == 0)//lost my chance
			{
				...
				//appropriately give coordinates of striker here!
				y_of_str=0.0f;
				while(1)
				{
	...
	if(isStrikerHitByOther=='y' && chance[3]=='n')//yahan pe aa rha h jo aana nahi chahiye
	{
		for(i=0;i<6;i++)
		{
			if(Vx[i]!=0.0f || Vy[i]!=0.0f)
		...
		if(cchek=='x')
		{
			...
			if(k == 0)
			{
				...
				{	
					chance[3]='y';
					chance[1]='n';chance[2]='n';chance[0]='n';
					//will have to give coordinates of striker here
					x_of_str=0.0f;
					while(1)
					{
