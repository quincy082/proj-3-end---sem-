void epsx(int i)
{
	...
	if(Vx[i] >= 0.0f)
	{
		eps = X[i] /*+ Vx[i]*dt*/ - 1.7;//printf("eps=%f Vx[0] befor collision=%f\n",eps,Vx[0]);
		X[i] = 1.7 -eps;
		Vx[i] = -Vx[i];
		//printf("x after collision=%f and Vx[0]=%f\n",X[i],Vx[0]);
		return;
	}
	if(Vx[i] <= 0.0f)
	{
		eps = fabs(X[i] /*+ Vx[i]*dt*/ + 1.7);
		X[i] = -1.7 + eps;
		Vx[i] = -Vx[i];
		return;
	}
...
void check_str_wall(float Vx[], float Vy[])
{
	...
	//Y[0] = Y[0] + Vy[0]*dt;
	//printf("life!!\n");
	if(X[0] /*+ Vx[0]*dt*/ >= 1.7f || X[0] /*+ Vx[0]*dt*/ <= -1.7f)//1.66f should be 1.69f
	{
		//printf("x before collision = %f and X[0]+Vx[0]*dt=%f\n",X[0],X[0]*Vx[0]*dt);
		epsx(0);
	}
	if(Y[0] /*+Vy[0]*dt*/ >= 1.7f || Y[0] /*+ Vy[0]*dt*/ <= -1.7f)
	{
		epsy(0);
	}
}
...
{
	//X[i] = X[i] + Vx[i]*dt;printf("i=%d vx*dt=%f vy*dt=%f\n",i,Vx[i]*dt,Vy[i]*dt);
	//Y[i] = Y[i] + Vy[i]*dt;printf("i=%d vx=%f vy=%f\n\n",i,Vx[i],Vy[i]);
	if(X[i] /*+Vx[i]*dt*/ >= 1.725f || X[i] /*+Vx[i]*dt*/ <= -1.725f)//1.69 should be actually 1.725
	{
		epsx(i);
	}
	if(Y[i] /*+ Vy[i]*dt*/ >= 1.725f || Y[i] /*+ Vy[i]*dt*/ <= -1.725f)
	{
		epsy(i);
	}
	return;
